...
class _KanbanSetStatePageState extends State<KanbanSetStatePage>
  ...
  getColumnData() async {
    try {
      final dio = Dio();
      var response =
          await dio.get("http://192.168.34.57/API/get_column_data_kanban.php");
  ...
  getTaskData() async {
    try {
      final dio = Dio();
      var response =
          await dio.get("http://192.168.34.57/API/get_task_data_kanban.php");
      ...
      if (response.statusCode == 200) {
        ...
        print(jsonEncode(tasksForColumns));
  ...
  void addColumn(String title) async {
    ...
    final dio = Dio();
    var url = 'http://192.168.34.57/API/add_column_kanban.php';
